...
<head>
    ...
    <?php $favicon = AFF_Config::getConfig( 'favicon' );
    if ( $favicon ) : ?>
...
<body>
    ...
    <script src="<?php echo AFF_URL ?>/admin/js/main.min.js"></script>
        ...
        <q-layout view="lHh Lpr lFf">
            <q-header elevated>
                <q-toolbar>
                    ...
                    <template v-if="!$q.platform.is.mobile"> <a :href="configs.site_url"
                            style="text-decoration: none; color:#fff; font-weight: bold;">Quay lại trang chủ</a>
                    </template>
            ...
            <q-drawer v-if="configs.user_id && user.aff_active == 1" v-model="leftDrawerOpen" show-if-above bordered
                :width="250" content-class="bg-grey-1">
                <q-scroll-area style="height: calc(100% - 150px); margin-top: 150px; ">
                    <q-list>
                        <template v-for="nav in navs">
                            <!-- <q-item v-if="nav.children.length == 0" :to="nav.to" class="text-bold" exact clickable > -->
                            <q-item v-if="!nav.hasOwnProperty('link')" :to="nav.to" class="text-bold" exact clickable
                                :key="nav.to">
                            ...
                            <q-item v-else class="text-bold" exact @click="openURL(nav.link)" clickable :key="nav.link">
                                <q-item-section avatar>
                                    <q-icon :name="nav.icon" />
                                </q-item-section>
                                <q-item-section>
          ...
          <q-item v-for="nav_child in nav.children" clickable :to="nav_child.to">
            ...
            <q-item-section>
              <q-item-label>{{nav_child.label}}</q-item-label>
                        ...
                        <q-item class="text-bold" exact @click="logout" clickable :key="logout">
                            <q-item-section avatar>
                                <q-icon name="logout" />
                            </q-item-section>
                            <q-item-section>
                ...
                <q-img class="absolute-top" src="https://cdn.quasar.dev/img/material.png" style="height: 150px">
                    <div class="absolute-bottom bg-transparent">
                        ...
                        <div v-if="Object.keys(settings).length">Cấp độ: <span
                                class="text-weight-bold">{{settings.commission_user_levels[user.level].name}}</span>
                        </div>
                    </div>
                </q-img>
            </q-drawer>
            ...
            <q-page-container class="padding-bottom-mb-50">
                <template v-if="$route.path !== '/dang-nhap' && $route.path !== '/dang-ky'">
                    ...
                    <q-btn color="primary" v-if="user.aff_active === '0'" icon="save" class="q-mb-lg q-ml-md"
                        label="Đăng xuất" @click="logout" />
                </template>

                <template name="fade" mode="out-in"
                    v-if="($route.path === '/dang-nhap' || $route.path === '/dang-ky') || user.aff_active === '1'">
                    <div class="doc-note doc-note--tip q-ma-md"
                        v-if="Object.keys(settings).length && settings.noti_all != '' && $route.path !== '/dang-nhap' && $route.path !== '/dang-ky'"
                        v-html="settings.noti_all">
                    </div>
                    <router-view></router-view>
                </template>
            </q-page-container>
            <q-tabs no-caps inline-label class="mobile-only bg-warning text-white shadow-2 fixed-bottom"
                v-if="configs.user_login != ''">
                <q-tab name="event_available" icon="child_care" label="Trang chủ" @click="openURL(configs.site_url)">
                </q-tab>
                <q-tab name="settings" icon="logout" label="Đăng xuất" @click="logout"></q-tab>
            </q-tabs>
